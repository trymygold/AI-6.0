...
const coShop = {
    ...
    connectToHost: function(hostId) { this.conn = this.peer.connect(hostId); this.conn.on('open', () => { showToast("Connected! Waiting for video..."); this.activateUI(); }); this.setupDataListener(); },
...
function fetchCategoryData(category) {
    ...
    const fetchPromise = new Promise(async (resolve, reject) => {
        try {
            ...
            const url = `https://www.googleapis.com/drive/v3/files?q='${folderId}' in parents and trashed = false and mimeType contains 'image/'&pageSize=1000&fields=files(id,name,thumbnailLink)&key=${API_KEY}`;
            ...
            JEWELRY_ASSETS[category] = data.files.map(file => {
                ...
                return { 
                    ...
                    thumbSrc: baseLink ? baseLink.replace(/=s\d+$/, "=s400") : `https://drive.google.com/thumbnail?id=${file.id}`, 
                    fullSrc: baseLink ? baseLink.replace(/=s\d+$/, "=s3000") : `https://drive.google.com/uc?export=view&id=${file.id}` 
                };
            });
...
faceMesh.onResults((results) => {
  ...
  if (results.multiFaceLandmarks && results.multiFaceLandmarks[0]) {
    ...
    updatePhysics(Math.atan2(rightEar.y - leftEar.y, rightEar.x - leftEar.x), lm[1].x, w);
...
hands.onResults((results) => {
  ...
  // --- 1. PLUGIN HOOK: If gesture file is loaded, run it ---
  if (window.JewelsGestures && results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
      window.JewelsGestures.detect(results.multiHandLandmarks[0]);
  }
  // ---------------------------------------------------------
...
/* --- EXPORTED HELPERS (Allow plugins to use these) --- */
window.changeProduct = function(direction) {
    ...
    if (newIndex >= list.length) newIndex = 0; if (newIndex < 0) newIndex = list.length - 1;
    applyAssetInstantly(list[newIndex], newIndex, true);
}
window.showToast = function(msg) { var x = document.getElementById("toast-notification"); x.innerText = msg; x.className = "show"; setTimeout(function(){ x.className = x.className.replace("show", ""); }, 3000); }

/* --- OTHER UTILS --- */
window.selectJewelryType = selectJewelryType; window.toggleTryAll = toggleTryAll; window.tryDailyItem = tryDailyItem; window.closeDailyDrop = closeDailyDrop; window.takeSnapshot = takeSnapshot; window.downloadAllAsZip = downloadAllAsZip; window.closePreview = closePreview; window.downloadSingleSnapshot = downloadSingleSnapshot; window.shareSingleSnapshot = shareSingleSnapshot; window.changeLightboxImage = changeLightboxImage; window.toggleCoShop = toggleCoShop; window.closeCoShopModal = closeCoShopModal; window.copyInviteLink = copyInviteLink; window.sendVote = (val) => coShop.sendVote(val);
