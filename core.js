/* core.js - Jewels-Ai: Base Engine (No Sharing Logic) */

/* --- CONFIGURATION --- */
const API_KEY = "AIzaSyAXG3iG2oQjUA_BpnO8dK8y-MHJ7HLrhyE"; 
const DRIVE_FOLDERS = {
  earrings: "1eftKhpOHbCj8hzO11-KioFv03g0Yn61n", chains: "1G136WEiA9QBSLtRk0LW1fRb3HDZb4VBD",
  rings: "1iB1qgTE-Yl7w-CVsegecniD_DzklQk90", bangles: "1d2b7I8XlhIEb8S_eXnRFBEaNYSwngnba"
};
...
let currentType = ''; let isProcessingHand = false, isProcessingFace = false;
let currentAssetName = "Select a Design"; let currentAssetIndex = 0; 
let physics = { earringAngle: 0, earringVelocity: 0, swayOffset: 0, lastHeadX: 0 };
let currentCameraMode = 'user'; 

/* Auto Try State */
let autoTryRunning = false; let autoTryIndex = 0; let autoTryTimeout = null;

/* Stabilizer */
const SMOOTH_FACTOR = 0.8; 
let handSmoother = { active: false, ring: { x:0, y:0, angle:0, size:0 }, bangle: { x:0, y:0, angle:0, size:0 } };

/* --- CO-SHOPPING ENGINE (Compact) --- */
const coShop = {
    peer: null, conn: null, myId: null, active: false, isHost: false, 
    init: function() { this.peer = new Peer(null, { debug: 2 }); this.peer.on('open', (id) => { this.myId = id; this.checkForInvite(); }); this.peer.on('connection', (c) => { this.handleConnection(c); showToast("Friend Connected!"); this.activateUI(); if (this.isHost) setTimeout(() => this.callGuest(c.peer), 1000); }); this.peer.on('call', (call) => { call.answer(); call.on('stream', (remoteStream) => { remoteVideo.srcObject = remoteStream; remoteVideo.style.display = 'block'; videoElement.style.display = 'none'; canvasElement.style.display = 'none'; showToast("Watching Host Live"); }); }); },
    ...
    handleConnection: function(c) { this.conn = c; this.setupDataListener(); }, callGuest: function(guestId) { const stream = canvasElement.captureStream(30); this.peer.call(guestId, stream); },
...
function updatePhysics(headTilt, headX) {
    const gravityTarget = -headTilt; physics.earringVelocity += (gravityTarget - physics.earringAngle) * 0.1; 
    physics.earringVelocity *= 0.92; physics.earringAngle += physics.earringVelocity;
    const headSpeed = (headX - physics.lastHeadX); physics.lastHeadX = headX;
    physics.swayOffset += headSpeed * -1.5; physics.swayOffset *= 0.85; 
...
function fetchCategoryData(category) {
    ...
    const fetchPromise = new Promise(async (resolve) => {
        try {
            ...
            JEWELRY_ASSETS[category] = data.files.map(file => ({ id: file.id, name: file.name, thumbSrc: file.thumbnailLink ? file.thumbnailLink.replace(/=s\d+$/, "=s400") : `https://drive.google.com/thumbnail?id=${file.id}`, fullSrc: file.thumbnailLink ? file.thumbnailLink.replace(/=s\d+$/, "=s3000") : `https://drive.google.com/uc?export=view&id=${file.id}` }));
            if (category === 'earrings') setTimeout(checkDailyDrop, 2000); resolve(JEWELRY_ASSETS[category]);
...
window.onload = async () => { initBackgroundFetch(); coShop.init(); await startCameraFast('user'); setTimeout(() => { loadingStatus.style.display = 'none'; }, 2000); await selectJewelryType('earrings'); };

async function selectJewelryType(type) {
  if (currentType === type && type !== undefined) return; currentType = type;
  startCameraFast((type === 'rings' || type === 'bangles') ? 'environment' : 'user'); 
  ...
  let assets = JEWELRY_ASSETS[type]; if (!assets) assets = await fetchCategoryData(type); if (!assets || assets.length === 0) return;
  assets.forEach((asset, i) => { const btnImg = new Image(); btnImg.src = asset.thumbSrc; btnImg.className = "thumb-btn"; btnImg.onclick = () => applyAssetInstantly(asset, i, true); container.appendChild(btnImg); });
...
async function applyAssetInstantly(asset, index, shouldBroadcast = true) {
    ...
    const highResImg = await loadAsset(asset.fullSrc, asset.id); if (currentAssetName === asset.name && highResImg) setActiveARImage(highResImg);
}
function highlightButtonByIndex(index) { const children = document.getElementById('jewelry-options').children; for (let i = 0; i < children.length; i++) { children[i].style.borderColor = (i === index) ? "var(--accent)" : "rgba(255,255,255,0.2)"; children[i].style.transform = (i === index) ? "scale(1.05)" : "scale(1)"; if(i===index) children[i].scrollIntoView({ behavior: "smooth", block: "nearest", inline: "center" }); } }

/* --- CAMERA & TRACKING --- */
async function startCameraFast(mode = 'user') {
    if (!coShop.isHost && coShop.active) return; if (videoElement.srcObject && currentCameraMode === mode && videoElement.readyState >= 2) return;
    currentCameraMode = mode; if (videoElement.srcObject) videoElement.srcObject.getTracks().forEach(track => track.stop());
    ...
    videoElement.srcObject = stream; videoElement.onloadeddata = () => { videoElement.play(); detectLoop(); };
}
...
faceMesh.onResults((results) => {
  ...
  canvasElement.width = w; canvasElement.height = h; canvasCtx.save();
  ...
  if (results.multiFaceLandmarks && results.multiFaceLandmarks[0]) {
    ...
    updatePhysics(Math.atan2(rightEar.y - leftEar.y, rightEar.x - leftEar.x), lm[1].x);
...
hands.onResults((results) => {
  const w = videoElement.videoWidth; const h = videoElement.videoHeight;
  if (window.JewelsGestures && results.multiHandLandmarks && results.multiHandLandmarks.length > 0) window.JewelsGestures.detect(results.multiHandLandmarks[0]);
  ...
  if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
      ...
      if (!handSmoother.active) { handSmoother = { active: true, ring: { x: mcp.x, y: mcp.y, angle: targetRingAngle, size: targetRingWidth }, bangle: { x: wrist.x, y: wrist.y, angle: targetArmAngle, size: targetBangleWidth } }; } 
      else {
          handSmoother.ring.x = lerp(handSmoother.ring.x, mcp.x, SMOOTH_FACTOR); handSmoother.ring.y = lerp(handSmoother.ring.y, mcp.y, SMOOTH_FACTOR); handSmoother.ring.angle = lerp(handSmoother.ring.angle, targetRingAngle, SMOOTH_FACTOR); handSmoother.ring.size = lerp(handSmoother.ring.size, targetRingWidth, SMOOTH_FACTOR);
          handSmoother.bangle.x = lerp(handSmoother.bangle.x, wrist.x, SMOOTH_FACTOR); handSmoother.bangle.y = lerp(handSmoother.bangle.y, wrist.y, SMOOTH_FACTOR); handSmoother.bangle.angle = lerp(handSmoother.bangle.angle, targetArmAngle, SMOOTH_FACTOR); handSmoother.bangle.size = lerp(handSmoother.bangle.size, targetBangleWidth, SMOOTH_FACTOR);
      }
...
function captureToGallery() {
    ...
    if (currentCameraMode === 'environment') { tempCtx.translate(0, 0); tempCtx.scale(1, 1); } else { tempCtx.translate(tempCanvas.width, 0); tempCtx.scale(-1, 1); }
    tempCtx.drawImage(videoElement, 0, 0); tempCtx.setTransform(1, 0, 0, 1, 0, 0);
    try { tempCtx.drawImage(canvasElement, 0, 0); } catch(e) {}
    
    // 2. Add Watermark/Text
    let cleanName = currentAssetName.replace(/\.(png|jpg|jpeg|webp)$/i, "").replace(/_/g, " "); cleanName = cleanName.charAt(0).toUpperCase() + cleanName.slice(1);
    const padding = tempCanvas.width * 0.04; const titleSize = tempCanvas.width * 0.045; const descSize = tempCanvas.width * 0.035; const contentHeight = (titleSize * 2) + descSize + padding;
    ...
    tempCtx.font = `bold ${titleSize}px Playfair Display, serif`; tempCtx.fillStyle = "#d4af37"; tempCtx.textAlign = "left"; tempCtx.textBaseline = "top"; tempCtx.fillText("Product Description", padding, tempCanvas.height - contentHeight);
    tempCtx.font = `${descSize}px Montserrat, sans-serif`; tempCtx.fillStyle = "#ffffff"; tempCtx.fillText(cleanName, padding, tempCanvas.height - contentHeight + (titleSize * 1.5));
    if (watermarkImg.complete) { const wWidth = tempCanvas.width * 0.25; const wHeight = (watermarkImg.height / watermarkImg.width) * wWidth; tempCtx.drawImage(watermarkImg, tempCanvas.width - wWidth - padding, padding, wWidth, wHeight); }
...
function takeSnapshot() {
    ...
    // CALL THE SHARE MODULE
    if (window.JewelsShare) window.JewelsShare.openPreview(data);
}

function toggleTryAll() { if (!currentType) { alert("Select category!"); return; } if (autoTryRunning) stopAutoTry(); else startAutoTry(); }
function startAutoTry() { autoTryRunning = true; autoTryIndex = 0; document.getElementById('tryall-btn').textContent = "STOP"; runAutoStep(); }
function stopAutoTry() { autoTryRunning = false; clearTimeout(autoTryTimeout); document.getElementById('tryall-btn').textContent = "Try All"; 
    // IF WE HAVE IMAGES, OPEN GALLERY
    if (window.JewelsShare) window.JewelsShare.showGallery();
}
...
async function runAutoStep() { 
    ...
    autoTryTimeout = setTimeout(() => { 
        triggerFlash(); 
        const data = captureToGallery();
        if (window.JewelsShare) window.JewelsShare.addToCollection(data);
        autoTryIndex++; 
        runAutoStep(); 
    }, 1500); 
}
...
/* --- UTILS & EXPORTS --- */
window.selectJewelryType = selectJewelryType; window.toggleTryAll = toggleTryAll; window.tryDailyItem = tryDailyItem; window.closeDailyDrop = closeDailyDrop; window.takeSnapshot = takeSnapshot; window.toggleCoShop = toggleCoShop; window.closeCoShopModal = closeCoShopModal; window.copyInviteLink = copyInviteLink; window.sendVote = (val) => coShop.sendVote(val); window.changeProduct = function(direction) { if (!JEWELRY_ASSETS[currentType]) return; const list = JEWELRY_ASSETS[currentType]; let newIndex = currentAssetIndex + direction; if (newIndex >= list.length) newIndex = 0; if (newIndex < 0) newIndex = list.length - 1; applyAssetInstantly(list[newIndex], newIndex, true); }; window.showToast = function(msg) { var x = document.getElementById("toast-notification"); x.innerText = msg; x.className = "show"; setTimeout(function(){ x.className = x.className.replace("show", ""); }, 3000); };
function checkDailyDrop() { const today = new Date().toDateString(); if (localStorage.getItem('jewels_daily_date') !== today && JEWELRY_ASSETS['earrings'].length > 0) { const list = JEWELRY_ASSETS['earrings']; const idx = Math.floor(Math.random() * list.length); dailyItem = { item: list[idx], index: idx, type: 'earrings' }; document.getElementById('daily-img').src = dailyItem.item.thumbSrc; document.getElementById('daily-name').innerText = dailyItem.item.name; document.getElementById('daily-drop-modal').style.display = 'flex'; localStorage.setItem('jewels_daily_date', today); } }
...
function toggleCoShop() { const modal = document.getElementById('coshop-modal'); if (coShop.myId) { document.getElementById('invite-link-box').innerText = window.location.origin + window.location.pathname + "?room=" + coShop.myId; modal.style.display = 'flex'; } else { window.showToast("Generating ID..."); } }
function closeCoShopModal() { document.getElementById('coshop-modal').style.display = 'none'; }
function copyInviteLink() { navigator.clipboard.writeText(document.getElementById('invite-link-box').innerText).then(() => window.showToast("Link Copied!")); }
